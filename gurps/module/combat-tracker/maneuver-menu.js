...
export const addManeuverMenu = async (html, combatant, token) => {
    ...
    if (typeof initiative === 'number')
        currentManeuver.classList.add('active');
    else
        currentManeuver.classList.remove('active');
    // Prepare tooltip.
    ...
    currentManeuver.addEventListener('contextmenu', async (event) => {
        ...
        if (currentManeuverName === 'do_nothing')
            return;
...
export const addManeuverListeners = () => {
    ...
    document.addEventListener('click', event => {
        if (event.target.classList?.contains('maneuver-badge') || event.target.closest('.maneuver-select-info'))
            return;
    ...
    document.addEventListener('click', event => {
        ...
        if (!event.target.classList.contains('maneuver-badge'))
            return;
