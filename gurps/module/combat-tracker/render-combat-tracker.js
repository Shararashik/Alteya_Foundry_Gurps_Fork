...
export async function renderCombatTracker(_app, element, _options, _context) {
    // @ts-expect-error: FVTT v12 compatibility
    if ((game.release?.generation ?? 12) < 13)
        element = element[0]; // FVTT v12 compatibility
    if (!element.classList.contains('bound')) {
        ...
        element.addEventListener('drop', function (ev) {
            ...
            if (!elementMouseIsOver)
                return;
            let combatantElement = elementMouseIsOver.closest('.combatant');
    ...
    if (!game.user)
        return;
    if (game.user.isGM) {
        const combatantElements = element.querySelectorAll('.combatant');
        combatantElements.forEach(li => {
            ...
            li.addEventListener('dragstart', ev => {
                ...
                let dragIcon = currentTarget.querySelector('.token-image');
                if (dragIcon)
