...
export class ModifierBucket extends Application {
    ...
    activateListeners($html) {
        ...
        if (this.isTooltip)
            globalModifier.addEventListener('mouseenter', event => this._onenter(event));
        ...
        modifierBucket?.addEventListener('wheel', event => {
            event.preventDefault();
    ...
    refreshPosition(element = this.element) {
        ...
        if (game.release.generation >= 13) {
            if (positionSetting === 'left') {
                ...
                const playersIsOverlapping = playerList.getBoundingClientRect().right > element[0].getBoundingClientRect().left + hotbarOffset;
    ...
    _injectHTML($html) {
        ...
        if (!bucketExists) {
            ...
            if (game.release.generation >= 13) {
                ...
                if (position === 'left')
                    document.querySelector('#ui-bottom > div').style.justifyContent = 'flex-start';
            }
            this._element = $html;
        }
        else {
