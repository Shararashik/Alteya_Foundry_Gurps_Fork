...
export class GurpsActorSheet extends ActorSheet {
    ...
    activateListeners(html) {
        ...
        // Click on a spell row name in the modern NPC sheet to toggle notes visibility.
        html.find('.ms-spells-row .ms-col-name').click(ev => {
            const row = $(ev.currentTarget).closest('.ms-spells-row');
            row.toggleClass('ms-show-notes');
        });
...
// Кнопка "−" (убрать 1 урон с локации)
html.find('.ms-loc-hp-dec').on('click', async (ev) => {
    ev.preventDefault();
    const locationKey = $(ev.currentTarget).data('location-key');
    if (!locationKey) return;
    const loc = foundry.utils.getProperty(this.actor.system, `hitlocations.${locationKey}`);
    if (!loc) return;
    const current = loc.localHP ?? 0;
    if (current <= 0) return;
    await this.actor.update({
        [`system.hitlocations.${locationKey}.localHP`]: current - 1
    });
});

// Кнопка "+" (добавить 1 урон на локацию)
html.find('.ms-loc-hp-inc').on('click', async (ev) => {
    ev.preventDefault();
    const locationKey = $(ev.currentTarget).data('location-key');
    if (!locationKey) return;
    const loc = foundry.utils.getProperty(this.actor.system, `hitlocations.${locationKey}`);
...
// Кнопка сброса урона локации
html.find('.reset-local-hp').on('click', async (ev) => {
    ev.preventDefault();
    const locationKey = $(ev.currentTarget).data('location-key');
    if (!locationKey) return;
         ...
         const confirmed = await Dialog.confirm({
             title: 'Reset Location Damage',
             content: `<p>Reset damage for this location?</p>`,
         });
         if (!confirmed) return;
	
    await this.actor.resetLocalHP(locationKey);
});

// Чекбокс калечения — ручное управление, независимо от localHP
html.find('.ms-loc-cripple-check').on('change', async (ev) => {
    const locationKey = $(ev.currentTarget).data('location-key');
    if (!locationKey) return;
    const checked = ev.currentTarget.checked;
    await this.actor.update({
        [`system.hitlocations.${locationKey}.localCrippled`]: checked
    });
});
    ...
    async _onDrop(event) {
        ...
        if (dragData.type === DragDropType.DAMAGE)
