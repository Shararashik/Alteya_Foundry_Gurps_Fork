...
export class GurpsActor extends Actor {
        ...
        this.system.conditions.reeling = false;
        {
            ...
            if (!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_USE_SIZE_MODIFIER_DIFFERENCE_IN_MELEE)) {
                if (sizemod !== '0' && sizemod !== '+0') {
                    this.system.conditions.target.modifiers.push(`${game.i18n.format('GURPS.modifiersSize', { sm: sizemod })} #hit @sizemod`);
                }
            }
            else {
                if (sizemod !== '0' && sizemod !== '+0') {
                    this.system.conditions.target.modifiers.push(`${game.i18n.format('GURPS.modifiersSize', { sm: sizemod })} #ranged @sizemod`);
                }
            }
        }
    ...
    calculateDerivedValues() {
		...
		// new for localHP
		this._calculateLocalHPThresholds();
    ...
    _initializeStartingValues() {
		...
		recurselist(data.hitlocations, (e, _k, _d) => {
			if (!e.dr)
				e.dr = e.import;
			if (e.localHP === undefined || e.localHP === null)
				e.localHP = 0;
			if (e.localCrippled === undefined || e.localCrippled === null)
				e.localCrippled = false;
		});
    }
	
	_calculateLocalHPThresholds() {
    const data = this.system;
    const hpMax = data.HP?.max ?? 0;
    if (!hpMax) return;

    const bodyplan = data.additionalresources?.bodyplan;
    const table = HitLocations.HitLocation.getHitLocationRolls(bodyplan);

    recurselist(data.hitlocations, (e, _k, _d) => {
        const role = table[e.where]?.role;

        if (role === HitLocations.LIMB) {
            e.localHPMax = Math.floor(hpMax / 2);
            e.hasLocalHP = true;
        } else if (role === HitLocations.EXTREMITY) {
            e.localHPMax = Math.floor(hpMax / 3);
            e.hasLocalHP = true;
        } else if (e.where === 'Eye') {
            e.localHPMax = Math.floor(hpMax / 10);
            e.hasLocalHP = true;
        } else {
            e.localHPMax = null;
            e.hasLocalHP = false;
        ...
        // Вычисляем статус калечения
        e.isCrippled = !!e.localCrippled;
    });
}
    ...
    async replaceManeuver(maneuverText) {
        ...
        if (!tokens || tokens.length === 0) {
            ...
            ui.notifications?.warn(game.i18n.format('GURPS.combat.ui.noActiveTokensToUpdateManeuver', { name: this.displayname }));
            return;
        }
    ...
    async addMoveMode(mode, basic, enhanced = basic, isDefault = false) {
        ...
        for (const k in move) {
            if (move[k].mode === mode) {
                ...
                if (isNewDefault)
                    Object.values(move).forEach(it => (it.default = false));
    ...
    async handleEquipmentDrop(dragData) {
        ...
        if (!!this.isOwner && !!srcActor.isOwner) {
            ...
            if (count >= eqt.count)
                await srcActor.deleteEntry(dragData.key);
            else
                await srcActor.updateEqtCount(dragData.key, +eqt.count - count);
        }
        else {
    ...
    get temporaryEffects() {
        ...
        if (!game.user?.isGM && !this.isOwner) {
            ...
            if (detail === 'General' || (detail === 'NoFeint' && maneuver?.flags.gurps?.name === 'feint')) {
                if (!!maneuver.flags.gurps?.alt)
                    maneuver.img = maneuver.getFlag('gurps', 'alt');
            }
        }
	...
	async resetLocalHP(locationKey) {
		const path = `system.hitlocations.${locationKey}.localHP`;
		await this.update({ [path]: 0 });
	}


async applyDamageToLocation(locationWhere, injuryAmount) {
    if (!injuryAmount || injuryAmount <= 0) return;
    
    const hitlocations = this.system.hitlocations;
    for (const [key, loc] of Object.entries(hitlocations)) {
        if (loc.where === locationWhere && loc.hasLocalHP) {
            const current = loc.localHP ?? 0;
            const newValue = current + injuryAmount;
            await this.update({
                [`system.hitlocations.${key}.localHP`]: newValue
            });
            return;
        }
    }
