    ...
    async importSkillsFromGCA(json) {
        ...
        for (let key in json) {
            if (key.startsWith('id-')) {
                ...
                if (!!j.pageref)
                    sk.pageRef(readXmlText(j.pageref));
                ...
                if (!!item.itemid) {
                    ...
                    if (!!i?.system.globalid) {
                        if (!(item instanceof Skill))
                            item = Skill.fromObject(item, this.actor);
    ...
    async importSpellsFromGCA(json) {
        ...
        for (let key in json) {
            if (key.startsWith('id-')) {
                ...
                sp.resist = readXmlText(j.resist);
                ...
                if (!!item.itemid) {
                    ...
                    if (!!i?.system.globalid) {
                        if (!(item instanceof Spell))
                            item = Spell.fromObject(item, this.actor);
                        item = await this._processItemFrom(item, 'GCA');
                        temp.push(item);
                    }
                }
    ...
    async importProtectionFromGCA(json) {
        ...
        for (let key in json) {
            if (key.startsWith('id-')) {
                ...
                if (i.match(/^\d+ *(\+ *\d+ *)?$/))
                    i = eval(readXmlText(j.dr)); // supports "0 + 8"
                hl.import = !i ? 0 : i;
