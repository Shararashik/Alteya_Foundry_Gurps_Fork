...
import { arrayToObject, atou, d6ify, flattenContainedList, makeRegexPatternFrom, objectToArray, quotedAttackName, recurselist, sanitize, stripBracketContents, utoa, wait, zeroFill, } from '../lib/utilities.js';
import { prepareRemoveKey } from './actor/deletion.js';
import { calculateRoFModifier } from './combat/utilities.js';
import { GurpsActorCombatSheet, GurpsActorEditorSheet, GurpsActorSheet, GurpsActorSheetReduced, GurpsActorSimplifiedSheet, GurpsActorTabSheet, GurpsInventorySheet, } from './actor/actor-sheet.js';
...
`;
    ...
    const actionFuncs = {
        ...
        async attack({ action, actor, event }) {
            ...
            if ('itemPath' in action)
                opt.itemPath = action.itemPath;
            let targetmods = [];
            ...
            if (parsedRateOfFire > 1) {
                ...
                if (bonusForRoF !== 0)
                    GURPS.ModifierBucket.addModifier(bonusForRoF, game.i18n.format('GURPS.combat.rof.bonusLabel', { shots }), targetmods);
                opt.shots = shots;
            }
            if (action.overridetxt)
    ...
    async function performAction(action, actor, event = null, targets = []) {
        ...
        if (action.sourceId) {
            ...
            if (!actor || actor.id === originalActor.id)
                actor = originalActor;
        }
    ...
    async function handleRoll(event, actor, options) {
        ...
        if ('damage' in element.dataset) {
            ...
            if ('otf' in element.dataset) {
                ...
                if ('action' in parsedLink)
                    action = parsedLink.action;
