...
if (this._calculator.resourceType === 'HP') {
    const hitLoc = this._calculator.hitLocation;
    const role = this._calculator.hitLocationRole;
    const isTrackableLocation = (
        role === 'limb' || 
        role === 'extremity' || 
        hitLoc === 'Eye'
    );
    if (isTrackableLocation && hitLoc && hitLoc !== 'Random' && hitLoc !== 'Large-Area') {
        await this.actor.applyDamageToLocation(hitLoc, injury);
    }
}
