...
export class MovementHud extends ArgonComponent {
  ...
  updateMovement() {
    ...
    // max === 0: movement forbidden by maneuver (e.g. Do Nothing)
    if (!max) {
    ...
    const disabledBars = this.movementUsed % max;
    const barsNumber = max - disabledBars;
    ...
    currentEl.innerText = barsNumber;
    maxEl.innerText = ((Math.floor(this.movementUsed / max) + 1) * max) || 0;
