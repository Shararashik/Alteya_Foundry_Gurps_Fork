...
export class CoreHud extends HandlebarsApplication {
  ...
  performModuleCheck() {
    // If a dedicated system module exists and is active — all good
    const systemModule = game.modules.get(
      `enhancedcombathud-${game.system.id}`
    );
    if (systemModule?.active) return;

    // If the system registered its own components via argonInit — also fine
    // (e.g. built-in GURPS integration in this module)
    const suppressWarnings = game.settings.get(
      "enhancedcombathud",
      "suppressWarnings"
    );
